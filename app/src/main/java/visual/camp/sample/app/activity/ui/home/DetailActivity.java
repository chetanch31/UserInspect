...
import visual.camp.sample.app.activity.RecordingActivity;
...
// Retrieve the values that were passed as extras
        ...
        txtDesc=findViewById(R.id.textView5);
        txtDesc.setText(taskDesc);



        initView();
        checkPermission();
//        btnInitGaze.performClick();
        initHandler();
    }
    ...
    private void showToast(final String msg, final boolean isShort) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(DetailActivity.this, msg, isShort ? Toast.LENGTH_SHORT : Toast.LENGTH_LONG).show();
            }
        });
    }
